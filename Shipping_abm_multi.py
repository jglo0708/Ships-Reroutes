...
class Ship(Agent):
    ...
    def routing(self):
        ...
        # if not_reached == (len(ports) - 1): #if no routes possible routes found, reassign destination
        #     self.origin_failed += 1
        #     self.destination = self.dest()
        #     self.ports =  [*self.start_port, *self.destination]
        #     if self.origin_failed > 1: #if the problem persists, the ship is stuck, we reassign the origin
        #         self.stuck += 1
        #         self.start_port = self.origin()
        #         return self.routing()
        #     else:
        ...
        #     print(flat_route, travel_distance)
